...
import { WrapperTotal, WrapperCountOrder, WrapperInfo, WrapperItemOrder, WrapperLeft, WrapperlistOrder,  WrapperRight, WrapperStyleHeader, WrapperStyleHeaderDelivery } from './style'
...
const OrderPage = () => {
  ...
  const diliverypriceMeno = useMemo(() => {
   if (priceMeno >= 250000 && priceMeno < 500000) {
    return 15000
   } else if( priceMeno >= 500000 || order?.orderItemsSelected?.length === 0) {
     return 0
   } else {
    return 30000
   }
  ...
  const itemsDelivery = [
    {
      title: '30.000 VND',
      description: 'Dưới 250.000 VND',
    },
    {
      title: '15.000 VND',
      description: 'Từ 250.000 VND đến dưới 500.000 VND',
      subTitle: '',
    },
    {
      title: '0 VND',
  ...
  return (
    <div style={{background: '#f5f5fa', width: '100%', height: '100vh'}}>
      ...
      <h3 style={{display: 'inline-block', fontSize: '18px',}}>Giỏ hàng</h3>
      <div style={{display: 'flex', justifyContent: 'center'}}>
        <WrapperLeft>
          <WrapperStyleHeaderDelivery>
            <StepComponent items={itemsDelivery} current={diliverypriceMeno === 15000
               ? 2 : diliverypriceMeno === 30000 
          ...
          <WrapperStyleHeader>
            ...
            <div style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
              ...
              <DeleteOutlined style={{cursor: 'pointer' ,fontSize: '20px', fontWeight: 600}} onClick={handleRemoveAllOrder}/>
            </div>
          </WrapperStyleHeader>
          <WrapperlistOrder>
            {order?.orderItems?.map((order) => {
               return (
                ...
                <div style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}>
                  <span>
                    <span style={{ fontSize: '15px', color: '#242424'}}>{converPrice(order?.price)}</span>
                  ...
                  <span style={{color: 'rgb(255, 66, 78', fontSize: '15px', fontWeight: 500}}>{converPrice(order?.price * order?.amount)}</span>
                  <DeleteOutlined style={{cursor: 'pointer', fontSize: '20px'}} onClick={() => handleDeleteOrder(order?.product)}></DeleteOutlined>
                </div>
              </WrapperItemOrder>
               )
            })}
        ...
        <WrapperRight>
          ...
          <WrapperInfo>
            <div>
              ...
              <span style={{ fontSize: '14px', fontWeight: 500, paddingLeft: '5px'}}>{`${user?.address} , ${user?.city}`}</span>
              <span onClick={handleChangeAddress} style={{ cursor: 'pointer'  ,color: 'blue', fontSize: '13px', fontWeight: 500, paddingLeft: '5px'}}>Thay đổi</span>
            </div>
            </WrapperInfo>
            <WrapperInfo>
